...
class PriceValueModel() : ModelInterface{
    ...
    override fun get(fieldName: String, fieldValue: String): RealmObject {
        val realm = Realm.getDefaultInstance()
        var result  = realm.where<PriceValue>().equalTo(fieldName,fieldValue).findFirst()
        return result as RealmObject
    }

    override fun getAll(fieldName: String?, fieldValue: String?): MutableList<RealmObject> {
        val realm = Realm.getDefaultInstance()
        var result : RealmResults<PriceValue>? = null
        if(fieldName == null && fieldValue == null){
            result = realm.where<PriceValue>().findAll()
        }else{
            result = realm.where<PriceValue>().equalTo(fieldName,fieldValue).findAll()
        }
        val list : MutableList<PriceValue> = mutableListOf()
        result.forEach {
