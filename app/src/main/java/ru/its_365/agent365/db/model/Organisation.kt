...
class OrganisationModel() : ModelInterface{
    ...
    override fun get(fieldName: String, fieldValue: String): RealmObject? {
        val realm = Realm.getDefaultInstance()
        var result  = realm.where<Organisation>().equalTo(fieldName,fieldValue).findFirst()
    ...
    override fun getAll(fieldName: String?, fieldValue: String?): MutableList<RealmObject> {
        val realm = Realm.getDefaultInstance()
        var result : RealmResults<Organisation>? = null
        if(fieldName == null && fieldValue == null){
            result = realm.where<Organisation>().findAll()
        }else{
            result = realm.where<Organisation>().equalTo(fieldName,fieldValue).findAll()
        }
        val list : MutableList<Organisation> = mutableListOf()
        result.forEach {
